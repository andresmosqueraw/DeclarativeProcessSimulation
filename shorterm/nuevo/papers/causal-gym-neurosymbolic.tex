...
\title{\textbf{Causal-Gym: A Neuro-Symbolic Environment for Distilling Prescriptive Policies}}
...
This diagram illustrates **Causal-Gym**, a framework that outperforms state-of-the-art methods by combining three distinct stages:
\begin{enumerate}
    \item \textbf{Initialization:} Loading AS-IS models and partial states.
    \item \textbf{Training (The Gym):} A runtime-injected simulation where an \textbf{RL Agent} explores actions, guided by \textbf{Causal Rewards} (to fix bias) and \textbf{Symbolic Guards} (to ensure safety).
    \item \textbf{Distillation:} The complex, slow agent is "distilled" into a lightweight \textbf{Interpretable Policy} (Decision Tree) for sub-millisecond production inference.
\end{enumerate}
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    % Inputs for RL
    \node[input] (agent) at (-8, 8) {RL Agent\\(Actor-Critic)};
    ...
    \draw[arrow, green!70] (models.south) -- node[midway, below] {3} (compute.north);
    ...
    % Engine Components
    \node[procstep, fill=orange!10] (sim-env) at (-0.5, 5) {\textbf{SimBPMEnv}\\(State Mgmt)};
    \node[procstep, fill=orange!10] (flow-mgr) at (-0.5, 2.5) {\textbf{Flow Manager}\\(Navigation)};
  ...
  \end{tikzpicture}
  \caption{The \textbf{Causal-Gym Architecture}. Unlike standard approaches that deploy heavy black-box models, this system uses a three-stage pipeline. (1) We prepare the environment. (2) We inject a \textbf{Causal-RL Agent} into the simulation runtime; this agent explores the solution space, optimizing for Net Gain using IPW-adjusted rewards while constrained by Symbolic Safety Rules. (3) The resulting high-performance (but slow) agent generates a massive experience buffer, which is \textbf{distilled} into a lightweight, interpretable Decision Tree. This final model beats the SOTA in profit (via RL) while matching the speed of simple heuristics (via Distillation).}
